...
The repository carries a curated uv spec at [`environments/uv/requirements.in`](environments/uv/requirements.in) plus the compiled lock at [`environments/uv/requirements.lock`](environments/uv/requirements.lock). They mirror the January 2026 Conda stack (Python 3.10) so that we can rebuild the same toolchain without relying on Conda.

[`environments/uv/requirements.lock`](environments/uv/requirements.lock) was generated thusly:
```
