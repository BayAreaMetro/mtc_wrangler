...
def standardize_and_write(
    ...
    Side Effects:
        Writes multiple files to OUTPUT_DIR:
            - {prefix}{county}_links_momo.hyper: Tableau Hyper format for visualization
            - {prefix}{county}_nodes_momo.hyper: Tableau Hyper format for visualization
...
def step1_download_osm_network(county: str) -> networkx.MultiDiGraph:
    ...
    print(f"======= STEP 1: Download OSM network for {county} =======")
    ...
    if initial_graph_file.exists():
        try:
            ...
            print(f"Loaded cached OSM graph from {initial_graph_file}")
            WranglerLogger.info(f"Graph has {g.number_of_edges():,} edges and {len(g.nodes()):,} nodes")
            print(f"Graph has {g.number_of_edges():,} edges and {len(g.nodes()):,} nodes")
            return g
        except Exception as e:
            WranglerLogger.warning(f"Could not read cached graph: {e}")
            print(f"Could not read cached graph: {e}")
    
    # Download new graph
    if county == 'Bay Area':
        WranglerLogger.info("Downloading network for Bay Area using bounding box...")
        print("Downloading network for Bay Area using bounding box...")
        ...
        print(f"Bounding box: west={bbox[0]:.6f}, south={bbox[1]:.6f}, east={bbox[2]:.6f}, north={bbox[3]:.6f}")
        g = osmnx.graph_from_bbox(bbox, network_type=OSM_network_type)
    else:
        WranglerLogger.info(f"Downloading network for {county}...")
        print(f"Downloading network for {county}...")
    ...
    print(f"Cached OSM graph to {initial_graph_file}")
    WranglerLogger.info(f"Downloaded graph has {g.number_of_edges():,} edges and {len(g.nodes()):,} nodes")
    print(f"Downloaded graph has {g.number_of_edges():,} edges and {len(g.nodes()):,} nodes")
...
def step2_simplify_network_topology(g: networkx.MultiDiGraph, county: str) -> networkx.MultiDiGraph:
    ...
    print(f"======= STEP 2: Simplify network topology for {county} =======")
    ...
    if simplified_graph_file.exists():
        try:
            ...
            print(f"Loaded cached simplified graph from {simplified_graph_file}")
            WranglerLogger.info(f"Simplified graph has {simplified_g.number_of_edges():,} edges and {len(simplified_g.nodes()):,} nodes")
            print(f"Simplified graph has {simplified_g.number_of_edges():,} edges and {len(simplified_g.nodes()):,} nodes")
            return simplified_g
        except Exception as e:
            WranglerLogger.warning(f"Could not read cached simplified graph: {e}")
            print(f"Could not read cached simplified graph: {e}")
    ...
    print(f"Simplifying with tolerance={NETWORK_SIMPLIFY_TOLERANCE} feet...")
    ...
    print(f"Cached simplified graph to {simplified_graph_file}")
    WranglerLogger.info(f"Simplified graph has {simplified_g.number_of_edges():,} edges and {len(simplified_g.nodes()):,} nodes")
    print(f"Simplified graph has {simplified_g.number_of_edges():,} edges and {len(simplified_g.nodes()):,} nodes")
...
def step3_standardize_attributes(g: networkx.MultiDiGraph, county: str) -> Tuple[gpd.GeoDataFrame, gpd.GeoDataFrame]:
    ...
    print(f"======= STEP 3: Standardize attributes for {county} =======")
    ...
    print(f"Standardized network: {len(links_gdf):,} links, {len(nodes_gdf):,} nodes")
    return (links_gdf, nodes_gdf)
...
def step4_assign_county_node_link_numbering(links_gdf: gpd.GeoDataFrame, nodes_gdf: gpd.GeoDataFrame, county: str):
    ...
    print(f"======= STEP 4: Create roadway network with county numbering for {county} =======")
    ...
    try:
        ...
        print(f"Loaded cached roadway network from {roadway_net_file}")
    ...
    for roadway_format in ROADWAY_OUTPUT_FORMATS:
        try:
            ...
            print(f"Saved roadway network in {roadway_format} format")
        except Exception as e:
            WranglerLogger.error(f"Error writing roadway network in {roadway_format}: {e}")
            print(f"Error writing roadway network in {roadway_format}: {e}")
    ...
    print(f"Created roadway network with {len(roadway_network.links_df)} links and {len(roadway_network.nodes_df)} nodes")
    return roadway_network
...
def step5_integrate_gtfs_transit_data(county: str):
    ...
    print(f"======= STEP 5: Integrate GTFS transit data for {county} =======")
    ...
    if gtfs_model_dir.exists():
        try:
            ...
            print(f"Loaded cached GTFS model from {gtfs_model_dir}")
            return gtfs_model
        except Exception as e:
    ...
    print("Loading GTFS feed for September 27, 2023...")
    ...
    if county == "Bay Area":
        ...
        print(f"Keeping agencies for {county}: {keep_agencies}")
    ...
    print(f"Integrated GTFS data: {len(gtfs_model.routes)} routes, {len(gtfs_model.stops)} stops")
    return gtfs_model
...
def step6_create_transit_stops_and_links(gtfs_model, roadway_network, county: str):
    ...
    print(f"======= STEP 6: Create transit stops and links for {county} =======")
    ...
    try:
        ...
        print(f"Created transit feed with stops and links integrated")
        return feed
...
def step7_output_final_network_files(roadway_network, feed, county: str):
    ...
    if 'ML_access' in roadway_network.links_df.columns:
        ...
        print("Converting ML_access column to boolean")
    ...
    for roadway_format in ROADWAY_OUTPUT_FORMATS:
        try:
            ...
            print(f"Wrote roadway network with transit in {roadway_format} format")
        except Exception as e:
    ...
    print(f"Wrote transit feed to {feed_dir}")
    ...
    print(f"Created final transit network with {len(transit_network.feed.routes)} routes")
    
    return transit_network

# =============================================================================
# MAIN EXECUTION WITH 7-STEP WORKFLOW
# =============================================================================
...
if __name__ == "__main__":
    ...
    # Setup logging
    county_no_spaces = args.county.replace(" ", "")
    INFO_LOG = OUTPUT_DIR / f"create_mtc_network_from_OSM_{county_no_spaces}.info.log"
    DEBUG_LOG = OUTPUT_DIR / f"create_mtc_network_from_OSM_{county_no_spaces}.debug.log"
    ...
    try:
        ...
        g = step1_download_osm_network(args.county)
        
        # STEP 2: Simplify network topology
        simplified_g = step2_simplify_network_topology(g, args.county)
        
        # STEP 3: Standardize attributes
        (links_gdf, nodes_gdf) = step3_standardize_attributes(simplified_g, args.county)
        
        # STEP 4: Assign county-specific numbering
        roadway_network = step4_assign_county_node_link_numbering(links_gdf, nodes_gdf, args.county)
        
        # STEP 5: Integrate GTFS transit data
        gtfs_model = step5_integrate_gtfs_transit_data(args.county)
        
        # STEP 6: Create transit stops and links
        feed = step6_create_transit_stops_and_links(gtfs_model, roadway_network, args.county)
        
        # STEP 7: Output final network files
        transit_network = step7_output_final_network_files(roadway_network, feed, args.county)
